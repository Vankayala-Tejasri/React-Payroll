...
app.put('/employee_update/:code', async (req, res) => {
  ...
  try {
    ...
    const formattedJoinDate = emp_join_date ? new Date(emp_join_date).toISOString().slice(0, 10) : null;
    const formattedResignationDate = emp_resignation_date ? new Date(emp_resignation_date).toISOString().slice(0, 10) : null;
